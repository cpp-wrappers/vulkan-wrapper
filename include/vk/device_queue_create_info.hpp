...
struct device_queue_create_info {
	...
	device_queue_create_info(const Ps&... ps) {
		u::params{ ps... }
			.template handle<u::required<const queue_family_index&>>([&](auto i) {
				m_queue_family_index = i.value;
			})
			.template handle<u::required<const queue_count&>>([&](auto c) {
				m_queue_count = c.value;
			})
			.template handle<u::required<const queue_priorities&>>([&](auto p) {
