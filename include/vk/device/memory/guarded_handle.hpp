...
template<>
struct guarded_handle<vk::device_memory> : vk::guarded_device_child_handle_base<vk::device_memory> {
...
vk::result guarded_handle<vk::device_memory>::try_map(Args&&... args) const {
	return vk::try_map_device_memory(device(), handle(), forward<Args>(args)...);
}

template<typename... Args>
void guarded_handle<vk::device_memory>::map(Args&&... args) const {
	vk::map_device_memory(device(), handle(), forward<Args>(args)...);
}

#include "unmap.hpp"

inline void guarded_handle<vk::device_memory>::unmap() const {
...
template<typename... Args>
void guarded_handle<vk::device_memory>::flush_mapped(Args&&... args) const {
