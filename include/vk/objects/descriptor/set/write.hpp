	...
	struct write_descriptor_set {
		...
		requires (
			types::are_exclusively_satsify_predicates<
				...
				types::are_contain_one_type<vk::dst_binding>,
				types::are_contain_one_type<vk::dst_array_element>,
			...
			(
				types::count_of_ranges_of_value_type<vk::descriptor_image_info>::for_types_of<Args...> + 
				types::count_of_ranges_of_value_type<vk::descriptor_buffer_info>::for_types_of<Args...> +
				types::count_of_ranges_of_value_type<vk::handle<vk::buffer_view>>::for_types_of<Args...>
				== 1 
			)
		)
		write_descriptor_set(Args&&... args) {
			dst_set = vk::get_handle(elements::vk::possibly_guarded_handle_of<vk::descriptor_set>(args...));
			dst_binding = elements::of_type<vk::dst_binding>(args...);
			dst_array_element = elements::of_type<vk::dst_array_element>(args...);
			descriptor_type = elements::of_type<vk::descriptor_type>(args...);

			if constexpr(types::are_contain_range_of_value_type<vk::descriptor_image_info>::for_types_of<Args...>) {
				auto& r = elements::range_of_value_type<vk::descriptor_image_info>(args...);
				count = vk::count{ (uint32) r.size() };
				image_info = r.data();
			}

			if constexpr(types::are_contain_range_of_value_type<vk::descriptor_buffer_info>::for_types_of<Args...>) {
				auto& r = elements::range_of_value_type<vk::descriptor_buffer_info>(args...);
				count = vk::count{ (uint32) r.size() };
				image_info = r.data();
			}

			if constexpr(types::are_contain_range_of_value_type<vk::handle<vk::buffer_view>>::for_types_of<Args...>) {
