...
namespace vk {
	...
	void get_features(Args&&... args) {
		...
		if constexpr (types::are_contain_decayed<vk::physical_device_features>::for_types<Args...>) {
			features.features = elements::decayed<vk::physical_device_features>(args...);
		}

		const void** next = &features.next;

		elements::for_each_decayed_satisfying_predicate<vk::is_physical_device_features>(args...)(
			[&](auto& features) {
				*next = &features;
				next = &features.next;
			}
