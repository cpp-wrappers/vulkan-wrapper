...
namespace vk {
	...
	struct write_descriptor_set {
		...
		requires types::are_exclusively_satsify_predicates<
			...
			types::are_may_contain_one_range_of<vk::handle<vk::buffer_view>>
		>::for_types<Args...> &&
		(
			...
			types::count_of_ranges_of<vk::handle<vk::buffer_view>>::for_types<Args...>
			== 1 
		)
		write_descriptor_set(Args&&... args) {
			...
			if constexpr(types::are_contain_range_of<vk::handle<vk::buffer_view>>::for_types<Args...>) {
				auto& r = elements::range_of<vk::handle<vk::buffer_view>>(args...);
				count = vk::count{ (uint32) r.size() };
